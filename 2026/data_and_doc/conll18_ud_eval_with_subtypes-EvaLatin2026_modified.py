...
def evaluate(gold_ud, system_ud):
    ...
    def alignment_score_by_deprel(alignment, metric="LAS"):
        """Compute per-deprel F1 breakdown for LAS or CLAS.
        Returns a dict {deprel: Score} sorted by deprel name.
        For CLAS only content deprels are considered (both gold and system).
        """
        ...
        # gold_total[deprel]: count of gold words with that deprel (optionally filtered)
        gold_counts   = defaultdict(int)
        system_counts = defaultdict(int)
        correct_counts = defaultdict(int)

        for gw in alignment.gold_words:
            dep = gw.columns[DEPREL]
            if is_clas and not gw.is_content_deprel:
                continue
            gold_counts[dep] += 1

        for sw in alignment.system_words:
            dep = sw.columns[DEPREL]
            if is_clas and not sw.is_content_deprel:
                continue
            system_counts[dep] += 1

        for words in alignment.matched_words:
            gw, sw = words.gold_word, words.system_word
            gold_dep = gw.columns[DEPREL]
            if is_clas and not gw.is_content_deprel:
                continue
            # LAS/CLAS: HEAD must match AND deprel must match
            if gold_aligned_gold(gw.parent) == gold_aligned_system(sw.parent) \
               and gold_dep == sw.columns[DEPREL]:
                correct_counts[gold_dep] += 1

        all_deprels = sorted(set(list(gold_counts.keys()) + list(system_counts.keys())))
        result = {}
        for dep in all_deprels:
            result[dep] = Score(gold_counts[dep], system_counts[dep], correct_counts[dep],
                                aligned_total=None)
        return result
    ...
    # Compute per-deprel breakdowns (always computed, used when --breakdown is set)
    las_by_deprel  = alignment_score_by_deprel(alignment, metric="LAS")
    clas_by_deprel = alignment_score_by_deprel(alignment, metric="CLAS")
    ...
    scores["LAS_by_deprel"]  = las_by_deprel
    scores["CLAS_by_deprel"] = clas_by_deprel
    return scores
...
def main():
    ...
    parser.add_argument("--subtypes", "-s", default=False, action="store_true",
                        help="Include subtypes in the metric.")
    parser.add_argument("--counts", "-c", default=False, action="store_true",
                        help="Print raw counts of correct/gold/system/aligned words instead of prec/rec/F1 for all metrics.")
    parser.add_argument("--breakdown", "-b", default=False, action="store_true",
                        help="Print per-deprel F1 breakdown for LAS and CLAS.")
    args = parser.parse_args()
    if args.subtypes:
        global includeSubtypes 
    ...
    if args.breakdown:
        for metric_name, key in [("LAS", "LAS_by_deprel"), ("CLAS", "CLAS_by_deprel")]:
            print()
            print("Per-deprel breakdown: {}".format(metric_name))
            print("{:<20} | {:>9} | {:>9} | {:>9} | {:>7} | {:>7} | {:>7}".format(
                "Deprel", "Gold", "Predicted", "Correct", "Prec", "Rec", "F1"))
            print("-" * 80)
            by_deprel = evaluation[key]
            for dep in sorted(by_deprel.keys()):
                s = by_deprel[dep]
                print("{:<20} | {:>9} | {:>9} | {:>9} | {:>6.2f}% | {:>6.2f}% | {:>6.2f}%".format(
                    dep,
                    s.gold_total,
                    s.system_total,
                    s.correct,
